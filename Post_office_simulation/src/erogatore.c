 ...
 * Ho scelto le Message Queue perché permettono lo scambio di struct
...
int main(void) {
    ...
    while (1) {
        ...
        if (bytes == -1) {
            ...
            // La coda è stata distrutta quindi è ora di terminare anche l'erogatore
            if (errno == EIDRM) {
                // È il segnale che la simulazione è finita ed esco
                break; 
            }
        ...
        // Assegno il numero successivo
        msg.numero_ticket = global_ticket_counter++;

        // --- ROUTING DELLA RISPOSTA ---
        // Il punto importante: Cambio l'mtype
